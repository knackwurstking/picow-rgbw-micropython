...
def handle_request(req: str):
    ...
    if pathname.startswith("/set_rgbw_pin") and (method.upper() == "POST"):
        header, body = handler.set_rgbw_pin(parse_query(query))

    # POST: "/set_rgbw_pwm" ? r=<0-100%> & g=<0-100%> & b=<0-100%> & w=<0-100%>
    elif pathname.startswith("/set_rgbw_pwm") and (method.upper() == "POST"):
        header, body = handler.set_rgbw_pwm(parse_query(query))
